...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            ...
            max_val = -999999
            for point in dist:
                if dist[point] > max_val:
                    temp_pos = point
                    max_val = dist[point]
            next_dist = self.distancer.getDistance(pacmanPosition, temp_pos)
        ...
        action = None
        for act in legal:
            next_position = Actions.getSuccessor(pacmanPosition, act)
            temp_distance = self.distancer.getDistance(next_position, ghost_position)
