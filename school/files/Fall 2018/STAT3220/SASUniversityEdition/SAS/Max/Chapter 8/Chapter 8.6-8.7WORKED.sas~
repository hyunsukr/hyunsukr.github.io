...
proc reg data=homesales6 plots(label)=(cooksd rstudentbypredicted
rstudentbyleverage);
model price = size quality quality2 schooldum1 schooldum2 / r influence;
run;
...
*Outliers in x: check for leverages > 2(5+1)/25=.48 
-> Observation 10;
*Outliers in y: check for student residuals > 2 
-> No outliers in y;
*Influential Points:
1. Check Studentized Deleted Residuals > t(n-k-2,alpha/2);
data teststat;
t=quantile('T',0.975,25-5-2);
run;
proc print data=teststat;
run;
*2.10092;
*-> No influential observations according to studentized deleted residuals;
*2. Check Cook's Distance: Cook's Distance > F(k+1,n-k-1,alpha);
data teststat;
f=quantile('F',0.5,5+1,25-5-1);
run;
proc print data=teststat;
run;
*0.92375;
*-> No influential observations according to Cook's Distance;
...
model value = t / dwprob;
run;

*Pr<DW is the p-value for testing positive autocorrelation,
and Pr>DW is the p-value for testing negative autocorrelation;

*We detect positive autocorrelation at an alpha=0.05 level;

*Since there is autocorrelation, we must account for it in the model
We will begin learn in Ch 10 how to do that.;
...
*QUESTION: Are there any outliers or influential points;
proc reg data=prescript plots(only)=(cooksd rstudentbypredicted
rstudentbyleverage);
model Sales = FloorSp PresPct / r influence;
run;

*Outliers in x: check for leverages > 2(2+1)/20=0.3
-> none, but Observation 17 is close;
*Outliers in y: check for student residuals > 2 
-> Observations 5;
*Influential Points:
1. Check Studentized Deleted Residuals > t(n-k-2,alpha/2)
...
data teststat;
t=quantile('T',0.975,20-2-2);
f=quantile('F',0.5,2+1,20-2-1);
run;
proc print data=teststat;
run;
*t=2.11991;
...
*QUESTION: Are there any outliers or influential points;
proc reg data=soda2 plots(only)=(cooksd rstudentbypredicted
rstudentbyleverage);
model bubbles = temp temp2 / r influence;
run;

*Outliers in x: check for leverages > 2(2+1)/18=0.33
-> none;
...
data teststat;
t=quantile('T',0.975,18-2-2);
f=quantile('F',0.5,2+1,18-2-1);
run;
proc print data=teststat;
run;
*t=2.14479;
...
*QUESTION: Are there any outliers or influential points;
proc reg data=branding2 plots(only)=(cooksd rstudentbypredicted
rstudentbyleverage);
model likeness = moisture sweetness moistsweet / r influence;
run;

*Outliers in x: check for leverages > 2(3+1)/15=0.533
-> none;
...
data teststat;
t=quantile('T',0.975,15-3-2);
f=quantile('F',0.5,3+1,15-2-1);
run;
proc print data=teststat;
run;
*t=2.22814;
...
*QUESTION: Are there any outliers or influential points?;
proc reg data=heat plots(only)=(cooksd rstudentbypredicted 
rstudentbyleverage);
model pitch = soaktime difftime / r influence;
run;

*Outliers in x: check for leverages > 2(2+1)/32=0.1875
-> Observations 31 & 32;
...
data teststat;
t=quantile('T',0.975,32-2-2);
f=quantile('F',0.5,2+1,32-2-1);
run;
proc print data=teststat;
run;
*t=2.04841;
