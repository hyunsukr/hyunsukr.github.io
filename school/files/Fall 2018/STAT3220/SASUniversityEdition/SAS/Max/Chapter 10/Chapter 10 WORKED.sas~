...
*There is a linear growth long-run trend;
...
y_t = T_t + R_t --> y_t = Beta_0 + Beta_1*t + R_t
;
...
*95% PI: We are 95% confident that the beer production for 
the first quarter of 1960 is between 237.5920 and 260.9172;
...
model produced = time;
run;
*There seems to be a violation of independence since there is a 
...
*There seems to be no autocorrelation;
...
*What is the appropriate model for the historic quantity of craft beer produced in Australia?
*y(t)=Beta_0 + Beta_1*time + e(t) is an appropriate model;
...
*There is an increasing trend in the long run;
...
linear: y_t = Beta_0 + Beta_1*t + R_t
OR 
quadratic: y_t = Beta_0 + Beta_1*t + Beta_2*t^2 + R_t
;
...
*Overall significant, Adj-R^2= 0.7222, s= 78.50490;
...
*Overall, significant, Adj-R^2= 0.8214, s= 62.93455;
...
*The quadratic model has smaller prediction intervals;
...
*Possible violation of normality, possible pattern in the residuals, implying non-constant
variance or violation of independence;
...
* quadratic model with autoregressive errors:

y_t = Beta_0 + Beta_1*t + Beta_2*t^2 + phi*R_t-1 + e_t
...
* 
y-hat_t = 197.10558 - 5.14686*t + 0.46636*t^2 - 0.55440*R-hat_t-1
;
