...
~ NOTE: the notation for sum of squares is a bit different
	...
	- within sample variation: SSE/(n1+n2+n...-p)
		> SSE = variability unexplained by the difference 
		in sample means
	- between sample variation: SST/(p-1)
		> SST = variability explained by the difference 
		in sample means
	- SS(Total) = SSE + SST
	- [SST/(p-1)]/[SSE/(n1+n2+n...-p)] ~ F(p-1, n1+n2+n...-p)
...
*Completely randomized design= independent samples were
collected from three independent populations. There is one factor,
solvent type, with three levels: aromatics, chloroalkanes, and esters.
Therefore, there are three treatments: aromatics, 
chloroalkanes, and esters. It is not balanced because each 
treatment has a different number of experimental units.;
...
*It appears there are difference in the mean sorption rate for
the three solvents, so we will perform a statistical analysis
to compare the means, through the ANOVA procedure.;
...
* F=24.51, reject H0. There is evidence that
at least one mean differs from at least one other.;
...
* Again, F=24.51, reject H0. There is evidence that
at least one mean differs from at least one other.;
...
*Critical Studentized Range: 3.49256. See plot for differences.
Aromatics and Chloroalkanes is the only pair of solvents
that do not have a significantly different rate;
...
class solvent;
model rate=solvent;
means solvent/ tukey lines alpha=0.1;
run;

*Critical Studentized Range: 3.02129. See table for differences.
Aromatics and Chloroalkanes is the only pair of solvents
that do not have a significantly different rate;
...
*The response variable appears approximately normal;
...
*Reject equal variance. The experiment is not balanced,
so we will try a transformation.;
...
*The square root transformation of rate satisfies the assumptions;
...
class solvent;
model sqrtrate=solvent;
means solvent/ tukey cldiff ;
run;

*We are 95% confident that the true difference of the mean
of the sqare root of sorption rate between aromatics and
esters solvent types is between 0.23978	and 0.61291.;
...
*Completely randomized design= subjects are randomly assigned to 
treatments. There is one factor, group, with four levels: AR, AC, A, and P.
Therefore, there are four treatments: AR, AC, A, P.
It is balanced because each treatment has the same number 
...
proc sgplot data=drinkers;
scatter x=grp y=score;
run;

proc sgplot data=drinkers;
	vline grp /response=score stat=mean markers datalabel;
run;
...
*It appears there are difference in the mean task scores for
the three solvents, so we will perform a statistical analysis
to compare the means, through the ANOVA procedure.;
...
*Computation method;
proc anova data=drinkers;
class grp;
model score=grp;
run;

* F=10.29, reject H0. There is evidence that
at least one mean differs from at least one other.;
...
*Critical Studentized Range: 3.79069. See plot for differences.
Group A is significantly different from the rest.;
...
*Critical Studentized Range: 4.19634. See plot for differences.
There are only significant differences between A and  AR, A and P ;
...
*The response variable does not appear normal, but ANOVA
is robust against violations of normality;
...
*Fail to reject equal variance. Additionally, 
the experiment is not balanced, so we can continue.;
...
*A-C = (-0.53693, -0.13580)
A-AC = (-0.40238, -0.00126)
A-AR = (-0.57693, -0.17580)

There is technically a significant difference between the group
with just alcohol and the group with caffeine, but it may not be
practically significant. The largest difference is when a 
monetary reward is present. This might suggest, that someone is
likely to "sober up" when money is on the line.;
